    ...
    if ($_FILES["file"]["error"] == 0) {
        ...
        if ($file) {
            while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
                ...
                $whatsapp = $emapData[0]; // Ganti dengan indeks yang sesuai
                $nama = $emapData[1]; // Ganti dengan indeks yang sesuai
                $kategori = $emapData[2]; // Ganti dengan indeks yang sesuai

                if (!empty($whatsapp) && !empty($nama)) {
                    // Periksa apakah nama sudah ada dalam tabel
                    $checkQuery = $conn->prepare("SELECT COUNT(*) FROM kontak WHERE nama = ?");
                    $checkQuery->bind_param("s", $nama);
                    $checkQuery->execute();
                    ...
                    if ($count == 0) {
                        ...
                        $insertQuery = $conn->prepare("INSERT INTO kontak (whatsapp, nama, kategori) VALUES (?, ?, ?)");

                        if (!$insertQuery) {
                            echo "Error: " . $conn->error . "<br>";
                        } else {
                            $insertQuery->bind_param("sss", $whatsapp, $nama, $kategori);
                        ...
                        echo "Nama '$nama' sudah ada dalam tabel, data tidak diinsert.<br>";
                    }
                }
            }
